...
(async () => {
  ...
  app.get("/cars", async (req: Request, res: Response) => {
    ...
    return res
      .status(200)
      .send(JSON.stringify(cars.filter((car) => car.make === make)));
  });
  ...
  app.get("/cars/:id", async (req: Request, res: Response) => {
    ...
    if (!car) {
      return res.status(400).send(`car with id ${id} not found`);
    }
  ...
  app.post("/cars", (req: Request, res: Response) => {
    ...
    if (!id || !type || !model || !make || !cost) {
      return res
        .status(400)
        .send(`a car must have a id, type, model, make and cost`);
    }

    // add
    const car = { id, type, model, make, cost };
    cars.push(car);
    return res.status(200).send(JSON.stringify(car));
  });
